...
class SPN:
	...
	def evaluate(self, data, labels=None,summ="", minibatch_size=1000, epoch=0, solution_set=[]):
		ms = minibatch_size
		tot_loss = 0
		a = 0 #start of data for a btach
		b = 0 #end of data for a batch
		runs = 1+(len(data)-1)//ms
		solution_choice = ['a', 'b', 'c', 'd', 'e']
		selection = [0, 0, 0, 0, 0]
		correct = 0
		indicator = 0
		test_writer = open("./test.out", "w")
		for i in range(runs):
			print i+1, "/", runs
			b = min(len(data), a + ms)
			...
			if summ == "":	
				...
				if (i % 5 == 0):
					...
					selection[4] = loss
					index, value = max(enumerate(selection), key=operator.itemgetter(1))
					is_same = False
					if solution_choice[index] == solution_set[indicator]:
						is_same = True
						correct += 1
					test_writer.write('Question ID: {3}; Prediction: {0}; Correct Answer: {1}; {4}; {2}.\n'.format(solution_choice[index], solution_set[indicator], is_same, indicator+1, selection))
					indicator += 1
